...
def friendly_api_error(exc: Exception) -> str:
    ...
    if status and status >= 500:
        return f"HTTP {status}：火山方舟服务暂时异常，请稍后重试。"
    if "Connection" in name or "connection" in str(exc).lower():
        return "无法连接火山方舟，请检查部署环境网络后重试。"
...
st.title("AI Resume Screening Copilot")
st.caption("MVP：同时评估两个岗位，只提供 HR 人工复核建议，不自动录用或淘汰。")

with st.sidebar:
    st.header("设置")
    configured_api_key = get_setting("ARK_API_KEY")
    if configured_api_key:
        api_key = configured_api_key
        st.success("火山方舟 API Key 已通过安全配置加载。")
    else:
    ...
    base_url = st.text_input(
        "ARK Base URL",
        value=get_setting("ARK_BASE_URL", DEFAULT_ARK_BASE_URL),
    )
    model = st.text_input("模型", value=get_setting("ARK_MODEL", DEFAULT_ARK_MODEL))
    if st.button("测试 API 连接", use_container_width=True):
        if not api_key:
            st.error("请先输入火山方舟 API Key。")
        else:
            try:
                reply = test_ark_connection(api_key=api_key, base_url=base_url, model=model)
